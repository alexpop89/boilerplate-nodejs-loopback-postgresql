...
function givenWebSocketServerConfig(customConfig?: Record<string, string | number>): Record<string, string | number> {
  ...
  if (config.cleanupEmptyChildNamespaces == null) config.port = defaults.cleanupEmptyChildNamespaces;
  return config;
}

function createWebSocketClient(app: RestApplicationLike & {wsServer: {httpServer: HttpServer}}, websocketNamespace?: string): Socket {
  const url = `${app.wsServer.httpServer.url}${websocketNamespace}`;
  return io(url, {path: '/socket.io'}); // Adjust options as needed
}

export async function setupApplication(websocketNamespace?: string): Promise<AppWithClientAndSocket> {
