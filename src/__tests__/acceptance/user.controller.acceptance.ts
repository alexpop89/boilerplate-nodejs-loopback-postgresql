...
describe('UserController', () => {
  ...
  it('can call /me if logged in', async () => {
    const response = await client.get('/me').set('Authorization', `Bearer ${token}`).expect(200);
    expect(response.body).to.have.property('firstName', mockUser.firstName);
    expect(response.body).to.have.property('lastName', mockUser.lastName);
    expect(response.body).to.have.property('email', mockUser.email);
  ...
  it('can request a refresh token if logged in', async () => {
    await client.post('/refresh-token').send({refreshToken: refreshToken}).expect(401);

    const response = await client.post('/refresh-token').send({refreshToken: refreshToken}).set('Authorization', `Bearer ${token}`).expect(200);

    expect(response.body).to.have.property('token');
  });

  it('can query their own user if logged in', async () => {
    await client.get(`/users/${createdUserId}`).set('Authorization', `Bearer ${token}`).expect(200);
  });

  it('can query their own roles if logged in', async () => {
    await client.get(`/users/${createdUserId}/roles`).set('Authorization', `Bearer ${token}`).expect(200);
  });
  ...
  it('can update their own values if logged in', async () => {
    ...
    const response = await client.get(`/users/${createdUserId}`).set('Authorization', `Bearer ${token}`).expect(200);
    expect(response.body).to.have.property('firstName', 'Foo2');
    expect(response.body).to.have.property('lastName', 'Bar2');
  });

  it('can logout', async () => {
    await client.post('/logout').set('Authorization', `Bearer ${token}`).expect(204);
  });
  ...
  it('cannot use an old refresh token after logout, will get HTTP STATUS 401', async () => {
    ...
    await client.post('/refresh-token').send({refreshToken: refreshToken}).set('Authorization', `Bearer ${token}`).expect(401);
  });
  ...
  it('cannot query their own user if not logged in, will get HTTP STATUS 403', async () => {
    ...
    await client.get(`/users/${newUserId}`).set('Authorization', `Bearer ${token}`).expect(403);
  });
  ...
  it('cannot update other users` values, will get HTTP STATUS 403', async () => {
    ...
    await client
      .patch(`/users/${responseSignUp.body.id}`)
      .set('Authorization', `Bearer ${token}`)
      .send({
        firstName: 'Foooooo',
        lastName: 'Barrrrrr',
