...
import {AUTHENTICATION_METADATA_KEY, AuthenticationComponent, registerAuthenticationStrategy} from '@loopback/authentication';
...
export class MainApplication extends BootMixin(ServiceMixin(RepositoryMixin(RestApplication))) {
  constructor(options: ApplicationConfig = {}) {
    ...
    this.bind('authentication.refreshToken.expirationDays').to(parseInt(process.env.REFRESH_TOKEN_EXPIRATION_DAYS ?? '7', 10));

    this.bind('metadata.AUTHENTICATION_METADATA_KEY').to(AUTHENTICATION_METADATA_KEY);
    this.bind('interceptors.AuthorizationInterceptor').toProvider(AuthorizationInterceptorProvider).apply(asGlobalInterceptor());
  }
}
