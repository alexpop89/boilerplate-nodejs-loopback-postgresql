...
export class WebSocketControllerFactory {
  ...
  constructor(
    ...
    this.ctx.bind('ws.controller').toClass(this.controllerClass).tag('websocket').inScope(BindingScope.CONTEXT);
  }

  async create(socket: Socket) {
    // Instantiate the controller instance
    this.controller = await this.ctx.get<{[method: string]: Function}>('ws.controller');
    await this.setup(socket);
    return this.controller;
  }

  async connect(socket: Socket) {
    const connectMethods = MetadataInspector.getAllMethodMetadata('websocket:connect', this.controllerClass.prototype) ?? {};
  ...
  registerSubscribeMethods(socket: Socket) {
    const regexpEventHandlers = new Map<RegExp[], (...args: unknown[]) => Promise<void>>();
    const subscribeMethods = MetadataInspector.getAllMethodMetadata<(string | RegExp)[]>('websocket:subscribe', this.controllerClass.prototype) ?? {};
    for (const m in subscribeMethods) {
      for (const t of subscribeMethods[m]) {
        const regexps: RegExp[] = [];
        if (typeof t === 'string') {
          socket.on(t, async (...args: unknown[]) => {
            let done: Function = (result: unknown) => null;
            if (typeof args[args.length - 1] === 'function') {
              done = args.pop() as Function;
            }
            const result = await invokeMethod(this.controller, m, this.ctx, args);
            done(result);
          });
        } else {
          regexps.push(t);
