import {Constructor, Context, CoreBindings, inject, Interceptor, InvocationContext, InvocationResult, Provider, ValueOrPromise} from '@loopback/core';
 ...
 */
export class AuthorizationInterceptorProvider extends OwnershipCheckerUtil implements Provider<Interceptor> {
  /**
  ...
  async intercept(invocationCtx: InvocationContext, next: () => ValueOrPromise<InvocationResult>): Promise<InvocationResult> {
    ...
    for (const role of roles) {
      if (await this.isPermissionAllowed(role, modelName, httpMethod, user.id, this._methodName, invocationCtx)) {
        allowed = true;
      }
    }
   ...
   */
  private getRequestObjectSomehow(invocationCtx: InvocationContext): Request | undefined {
    // eslint-disable-next-line @typescript-eslint/ban-ts-comment
    // @ts-ignore
  ...
  protected async isPermissionAllowed(
    ...
    for (const condition of role.conditions) {
      if ((modelName.endsWith(condition.modelName) || condition.modelName === '*') && (condition.permissions.includes(httpMethod) || condition.permissions.includes('*'))) {
        allowed = allowed || (await super.isPermissionAllowedByIdOwnership(role, modelName, httpMethod, currentUserId, methodName, invocationContext));
        allowed = allowed || (await super.isPermissionAllowedByOtherOwnership(role, modelName, currentUserId, invocationContext));
      }
    }
    return allowed;
  }
